...
chromosomes=c(seq(1,23),"X","Y","MT")
mydata=sequenza.extract(file=name,chromosome.list=chromosomes) ##I am excluding the GL "chromosomes" since they generate an error in sequenza :S

for (chr in 1:length(chromosomes)) {
pdf(paste0(id,"_sequenza/","chromosome",chromosomes[chr],"_view.pdf"))
try(chromosome.view(mut.tab = mydata$mutations[[chr]], baf.windows = mydata$BAF[[chr]], ratio.windows = mydata$ratio[[chr]], min.N.ratio = chr, segments = mydata$segments[[chr]], main = mydata$chromosomes[chr]))
dev.off()
}
